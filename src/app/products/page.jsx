...
import { ShoppingCart, Edit, Star, Heart, ChevronLeft, ChevronRight } from "lucide-react";
...
const Products = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error("Failed to load products, please try again in a few seconds!");
    } finally {
  ...
  if (loading) {
    return <Loader loading={true} className={'animate-spin w-16 h-16'}/>;
  }
  if (error) {
    return <Loader error={error} className={'animate-spin w-16 h-16 block items-center justify-center'}/>;
  }
